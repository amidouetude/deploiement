...
def historique(request):
    ...
    if request.method == 'POST':
        ...
        if 'download' in request.POST:
            ...
            moyenne = df_consommation['quantite'].mean()  # Moyenne
            total = df_consommation['quantite'].sum()    # Total
            min_val = df_consommation['quantite'].min()  # Minimum
            max_val = df_consommation['quantite'].max()  # Maximum

            

            moyenne_formatted = "{:.2f}".format(moyenne)
            total_formatted = "{:.2f}".format(total)
            min_val_formatted = "{:.2f}".format(min_val)
            max_val_formatted = "{:.2f}".format(max_val)
